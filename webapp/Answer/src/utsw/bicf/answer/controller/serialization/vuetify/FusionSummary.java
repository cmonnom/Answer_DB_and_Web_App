...
import utsw.bicf.answer.model.hybrid.FusionRow;

public class FusionSummary extends Summary<FusionRow> {
	
	public FusionSummary(ModelDAO modelDAO, OrderCase aCase, String uniqueIdField) {
		super(createRows(modelDAO, aCase), uniqueIdField);
	}

	private static List<FusionRow> createRows(ModelDAO modelDAO, OrderCase aCase) {
		List<FusionRow> rows = new ArrayList<FusionRow>();
		for (Variant variant : aCase.getVariants()) {
			rows.add(new FusionRow(variant));
