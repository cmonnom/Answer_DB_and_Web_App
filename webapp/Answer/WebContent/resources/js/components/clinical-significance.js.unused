                ...
                <v-layout row wrap>
                    <v-flex xs12 v-for="item in clinicalSignificance" :key="item.label">
                        <v-layout row wrap>
                            ...
                            <v-flex xs8 pt-0 class="caption" v-if="!item.readonly">
                                <v-text-field :textarea="true" v-model="item.text" class="mr-2 no-height pt-1 no-label"
                                    hide-details @input="handleInputChanged" :readonly="item.readonly">
                                </v-text-field>
                            </v-flex>
                            <v-flex xs8 pt-0 class="subheading readonly-clin-sign mt-2" v-if="item.readonly" align-start>
                                {{ item.text }}
                            </v-flex>
                        </v-layout>
                    </v-flex>
